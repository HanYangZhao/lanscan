...
def scan(arg_network=None, vendor=True, portscan=True):
    networks = Networks()
    networks.initialize()
    n = None  # type: Network
    ...
    # print(table.draw())
    data = []
    with open('lanscan.json', 'w') as jsonfile:
        for row in content:
            rowData = {"ip":row[0], "name":row[1], "mac":row[2], "alive":row[3], "vendor":row[4]}
            data.append(rowData)
